#!/bin/bash
echo ECS_CLUSTER=GitlabPoc >> /etc/ecs/ecs.config;echo ECS_BACKEND_HOST= >> /etc/ecs/ecs.config;
export PATH=/usr/local/bin:$PATH

sudo mkdir /efs

yum -y install jq nfs-utils

sudo mount -t nfs4 -o nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2 fs-d4ae759c.efs.us-east-1.amazonaws.com:/ /efs
echo 'fs-d4ae759c.efs.us-east-1.amazonaws.com:/ /efs nfs4 nfsvers=4.1,rsize=1048576,wsize=1048576,hard,timeo=600,retrans=2 0 0' | sudo tee -a /etc/fstab
mount -a
